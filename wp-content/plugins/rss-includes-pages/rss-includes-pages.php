...
Description: Include pages (not just posts) in RSS feeds. Particularly useful to those that use WordPress as a CMS. The <a href="http://infolific.com/technology/software-worth-using/include-pages-in-wordpress-rss-feeds/#pro-version" target="_blank">pro version</a> (just $5) gives you more control.
...
function rssip_plugin_meta( $links, $file ) { // add some links to plugin meta row
	if ( strpos( $file, 'rss-includes-pages.php' ) !== false ) {
		...
		$links = array_merge( $links, array( '<a href="http://infolific.com/technology/software-worth-using/include-pages-in-wordpress-rss-feeds/#pro-version" target="_blank">Pro Version (just $5)</a>' ) );
	}
	return $links;
}
...
<div class="wrap" style="padding-bottom: 5em;">
	...
	<p>By default posts and pages are set to be included in feeds. The <a href="http://infolific.com/technology/software-worth-using/include-pages-in-wordpress-rss-feeds/#pro-version" target="_blank">pro version</a> (just $5) allows you to change these options.</p>
	<div id="rssip-items">
		<form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
			<ul id="rssip_itemlist">
				...
				<?php
					echo "<div>";
						echo "<label class='side-label' for='rssip_posts'>Include Posts:</label>";
						echo "<input disabled='disabled' class='checkbox' type='checkbox' name='rssip_posts' id='rssip_posts' CHECKED />";
						echo "<br />";

						echo "<label class='side-label' for='rssip_pages'>Include Pages:</label>";
	...
	<div id="rssip-sb">
		<div class="postbox" id="rssip-sbone">
			...
			<div class="inside">
				...
				<p>This plugin adds pages to your RSS feed. The <a href="http://infolific.com/technology/software-worth-using/include-pages-in-wordpress-rss-feeds/#pro-version" target="_blank">pro version</a> (just $5) gives you a little more control.</p>
				<ol>
					<li>Select the options to the left to indicate what should be included in your RSS feeds.</li>
					<li>To exclude posts/pages, list the IDs separated by commas. Or to include posts/pages, list the IDs separated by commas. Note that excluding and including are mutually exclusive. If you specify both, exclude will win.</li>
				...
				<ol>
					...
					<li>Note that services such as Feedburner may not update your feed immediately so you may not see the effect of your options to the left for some time.</li>
				</ol>
			</div>
		</div>
