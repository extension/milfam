...
function display_quicklinks() {
    $bookmarks = get_bookmarks(array('category_name' => "Quick Links"));
    if (count($bookmarks) > 0) {
        $html .= '<div class="post quicklinks">';
        $html .= '<h2>Quick Links</h2>';
        $html .= quicklink_menu($bookmarks);
        $html .= "</div>";
        echo $html;
    }
}

function quicklink_menu($bookmarks) {
	$html .= "<ul id=\"quicklinks\">";
	foreach($bookmarks as $bookmark) :
        $html .= '<li><a href="' . $bookmark->link_url . '">' . $bookmark->link_name . '</a></li>' . "\n";
    endforeach;
    $html .= "</ul>";
	return $html;
}

function home_navigation($cat_name = false) {

	$bookmarks = get_bookmarks(array('category_name' => $cat_name, orderby => 'rating'));
	$i = 1;
	foreach($bookmarks as $bookmark) :
    $menu .= '<li class="menu_' . $i . '"><a href="' . $bookmark->link_url . '"><strong>' . $bookmark->link_name . '</strong><span>' . $bookmark->link_description . '</span></a></li>' . "\n";
    $i++;
  endforeach;
	echo $menu;
}

function populate_image_slider($cat_name = false) {

	$bookmarks = get_bookmarks(array('category_name' => $cat_name, orderby => 'rating'));
	foreach($bookmarks as $bookmark) :
    $slider .= '<img src="'. $bookmark->link_url . '" alt="' . $bookmark->link_description . '"/>' . "\n";
  endforeach;
	echo $slider;
}

function single_image($cat_name = false) {

	$bookmarks = get_bookmarks(array('category_name' => $cat_name));
	foreach($bookmarks as $bookmark) :
    $slider .= '<img src="'. $bookmark->link_url . '" alt="' . $bookmark->link_description . '"/>' . "\n";
  endforeach;
	echo $slider;
...
function footer_links($cat_name = false) {
	$bookmarks = get_bookmarks(array('category_name' => $cat_name, orderby => 'rating'));
	$footer .= "<ul>";
	foreach($bookmarks as $bookmark) :
	  $footer .= '<li><a href="' . $bookmark->link_url . '">' . $bookmark->link_name . '</a></li>' . "\n";
  endforeach;
  $footer .= "</ul>";
	echo $footer;
}
